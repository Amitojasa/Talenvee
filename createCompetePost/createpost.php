...
if(isset($_POST["submit"])) {
    ...
    if($_COOKIE['media']=='text'){
        mysqli_query($conn,"INSERT INTO `posts`(`caption`, `authorId`, `tags`,`type`) VALUES ('$caption',$uid,'$tags','compete')") or die(error_page());
...
// if everything is ok, try to upload file
    ...
    if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
        $time = date('Y-m-d');
        if($_COOKIE['media']=='image'){
            mysqli_query($conn,"INSERT INTO `posts`(`imageSrc`, `caption`, `authorId`, `tags`,`type`) VALUES ('$target_file','$caption',$uid,'$tags','compete')") or die(error_page());
        }elseif($_COOKIE['media']=='video'){
            mysqli_query($conn,"INSERT INTO `posts`(`videoSrc`, `caption`, `authorId`, `tags`,`type`) VALUES ('$target_file','$caption',$uid,'$tags','compete')") or die(error_page());
        }
        $last_id=mysqli_query($conn,"select LAST_INSERT_ID()") or die(error_page());
		$last_id=mysqli_fetch_array($last_id);
		$pid=$last_id[0];
